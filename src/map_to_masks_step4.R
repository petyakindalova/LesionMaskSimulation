...
#'@param brain_mask Brain mask file 
#'@param tempdir Path with all temperary files saved in step 3
#'@param mapping A dataframe with voxel locations for 'lesion' voxels - saved as paer of step 1.
#'@param n_subsets Number of subsets used to split lesion matrix
...
#'@return Saved nifti files for coefficients, standard errors, z-scores, outliers.
#'
mapping_fn = function(brain_mask, tempdir, mapping, n_subsets = NULL, n_covs, GLMmethod, outputdir) {
  ...
  if(is.null(n_subsets)) {
    ...
    for (i in 1:n_subsets) {
      ...
      fits_overall = c(fits_overall, output)
      print(length(fits_overall))
      print(mem_used())
    }
  ...
  temp_img_outliers[brain_mask!=0] = sum(abs(fits_overall[[1]]$pearson_resid) > 2)
  for (i in 1:length(fits_overall)) {
    temp_img_outliers[mapping[i]] = sum(abs(fits_overall[[i]]$pearson_resid) > 2)
    fits_overall[[i]]$pearson_resid = NULL
    gc()
    if(i %% 100 == 0) print (i)
  }
  
  temp_img_outliers[1:902629]  = as.nifti(temp_img_outliers, image_temp)
  filename_img = paste0("pearson_outliercounts_nvars_", n_covs, "_method_", GLMmethod)
  writeNIfTI(temp_img_outliers, filename_img)

  #outliers
  temp_img_outliers = image_temp
  temp_img_outliers[brain_mask!=0] = sum(abs(fits_overall[[1]]$stud_pearson_resid) > 2)
  for (i in 1:length(fits_overall)) {
    temp_img_outliers[mapping[i]] = sum(abs(fits_overall[[i]]$stud_pearson_resid) > 2)
    fits_overall[[i]]$stud_pearson_resid = NULL
    gc()
    if(i %% 100 == 0) print (i)
  }

  temp_img_outliers[1:902629]  = as.nifti(temp_img_outliers, image_temp)
  filename_img = paste0("stud_pearson_outliercounts_nvars_", n_covs, "_method_", GLMmethod)
  writeNIfTI(temp_img_outliers, filename_img)
  ...
  print("% infinite/converging voxels out of mask voxels per variable")
